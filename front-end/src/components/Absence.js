...
import { checkAth, getCookie, removeCookie } from "../firebase/firebaseFuntions";
...
export default function () {
  ...
  useEffect(() => {
    fetch('http://localhost:3001/api/absence')
  ...
  return getAuth(config).currentUser != null && typeAuth == "student" ? (
    <div className="w-full h-screen truncate">
      <div className="head flex justify-end space-x-3 shadow-md w-full h-14 bg-gray-800 p-1 pr-10 items-center">
        ...
        >
          <span className="text-white font-seimbold">@{getAuth(config).currentUser.uid.toString().toLowerCase().substring(0, 6)}</span>
